...
-已排序- Nested Loop Join -算法需要 N + N*M 次访问(每次访问读取一行) – 大表JOIN小表

	* With Inner Sequential Scan(FullScan)
	* With Inner Index Scan

-已排序- Hash Join -复杂度就是 O(M+N) – 类似大小的表join
...
-已排序- Merge join -唯一产生排序的联接算法, O(N*Log(N) + M*Log(M))-需排序, O(N+M)-已排序,我们是只挑选相同的元素。道理如下：
