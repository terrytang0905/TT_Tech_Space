...
Greenplum = Master + Slaver(分布式PG instance)
...
Doris = Mesa的存储引擎+Impala查询引擎的开源优化实现
...
正规的数据库产品里分2部分:
	...
	2) Storage有时候也要看一下Catalog，或者叫Metadata service
...
**C.存在大数据解决方案能够混合支持通常在不同系统内的在线服务与分析处理的负载.**
...
**3.1. HSAP(hybrid serving/analytical processing) challenge**
...
**3.2. 数据存储: 存储引擎设计Storage Design**
	...
	* 读写分离(Separation of Reads/Writes):同时支持高并发的读与高吞吐的写操作
	...
	Comments: tablet 则是分布式调度和存储的最小单元。
...
**3.3. 并行查询执行引擎**
	...
	* 可定制的调度策略：HOS将调度策略与上述的EC调度机制解耦。HOS将不同query请求的多个EC进行分组，构成Scheduling Group(调度组)。每个调度组共享自己的调度资源，而HOS则负责监控每个调度组的资源份额，保证资源的公平性，实现了资源隔离。
	...
	KeyWords: 执行上下文调度机制/HOS/自定义调度策略
...
- 行存：支持低延迟查询,对点查询和批量scan更友好(应用Hbase类简单场景)
- 列存：按列做聚合更高效，压缩更高效，适合OLAP分析型场景(应用Impala场景)
